...
def test_median_single(execute_single_instruction):
    assert execute_single_instruction(MedianInstruction, [1]) == [1, 1]


def test_median_multiple(execute_single_instruction):
    stack = [1, 2, 3, 4, 5]
    expected = stack[::]
    expected.append(floor(median(stack)))
    assert execute_single_instruction(MedianInstruction, stack) == expected


def test_median_multiple_keeps(execute_single_instruction):
    stack = [99, 1, 2, 3, 4, 5]
    expected = stack[::]
    expected.append(floor(median(stack)))
    assert execute_single_instruction(MedianInstruction, stack) == expected
